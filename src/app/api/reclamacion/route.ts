...
// Escapa caracteres HTML para evitar inyección de HTML/scripts en el correo.
...
export async function POST(req: NextRequest) {
  ...
  // Normaliza y valida la entrada del formulario público.
  ...
  if (nombre.length > 120 || dni.length > 20 || email.length > 150 || descripcion.length > 3000) {
  ...
  try {
    ...
    await resend.emails.send({
      ...
      to: [ADMIN_EMAIL],
      replyTo: email,
      subject: `📋 Libro de Reclamaciones - ${tipoLabel} de ${esc(nombre)}`,
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px;">
          ...
          <div style="background:#f8f8f8;padding:24px;border-radius:0 0 12px 12px;border:1px solid #eee;border-top:none;">
            <table style="width:100%;border-collapse:collapse;">
              <tr><td style="padding:8px 0;color:#666;font-size:13px;width:140px;">Nombre</td><td style="padding:8px 0;font-weight:bold;color:#111;">${esc(nombre)}</td></tr>
              <tr><td style="padding:8px 0;color:#666;font-size:13px;">DNI / RUC</td><td style="padding:8px 0;font-weight:bold;color:#111;">${esc(dni)}</td></tr>
              <tr><td style="padding:8px 0;color:#666;font-size:13px;">Email</td><td style="padding:8px 0;font-weight:bold;color:#111;">${esc(email)}</td></tr>
            ...
            <div style="margin-top:16px;padding:16px;background:white;border-radius:8px;border:1px solid #eee;">
              ...
              <p style="color:#111;font-size:14px;line-height:1.6;margin:0;">${esc(descripcion).replace(/\n/g, "<br/>")}</p>
            </div>
